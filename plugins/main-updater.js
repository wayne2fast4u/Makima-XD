...
lite({
    ...
    try {
        await reply("🔍 Checking for SUHO updates...");
        ...
        if (latestCommitHash === currentHash) {
            return reply("✅ Your SUHO MD  bot is already up-to-date!");
        }

        await reply("🚀 Updating SUHO MD  Bot...");

        // Download the latest code
        const zipPath = path.join(__dirname, "latest.zip");
        const { data: zipData } = await axios.get("https://github.com/NaCkS-ai/Sung-Suho-MD/archive/main.zip", { responseType: "arraybuffer" });
        ...
        const sourcePath = path.join(extractPath, "Sung-Suho-MD-main");
